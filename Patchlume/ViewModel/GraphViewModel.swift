...
final class GraphViewModel: ObservableObject {
    ...
    lazy var ndiOutputManager = NDIOutputManager(device: renderEngine.device)
    lazy var ndiSourceEngine = NDISourceEngine(device: renderEngine.device)
    // Shared by AutoDirectorEngine (automatic scene rotation) and
    // SceneBankView (a performer tapping a scene by hand) — one crossfade
    // implementation, and a manual tap correctly interrupts/overrides
    // whichever transition the director was already mid-way through.
    lazy var sceneCrossfader = SceneCrossfader(store: store)
    lazy var autoDirector = AutoDirectorEngine(store: store, audio: audio, cameraEngine: cameraEngine, crossfader: sceneCrossfader, ndiSourceEngine: ndiSourceEngine)
    ...
    init() {
        ...
        store.$graph.sink { [weak self] graph in
            ...
            if cameraNodes.isEmpty {
                ...
                let lensRaw = Int((cameraNodes.first?.parameters["p2"] ?? 1).rounded())
                let lens = CameraCaptureEngine.CameraLens(rawValue: lensRaw) ?? .back
                self.cameraEngine.setLens(lens)
            }
        ...
        renderEngine.ndiOutputManager = ndiOutputManager
        ndiOutputManager.onNeedsFrame = { [weak self] in self?.renderEngine.captureFrameForRecording() }
        ndiOutputManager.objectWillChange.sink { [weak self] _ in self?.objectWillChange.send() }.store(in: &cancellables)
        autoDirector.objectWillChange.sink { [weak self] _ in self?.objectWillChange.send() }.store(in: &cancellables)
        sceneCrossfader.objectWillChange.sink { [weak self] _ in self?.objectWillChange.send() }.store(in: &cancellables)
        renderEngine.ndiSourceTextureProvider = { [weak self] in self?.ndiSourceEngine.latestTexture }
        ndiSourceEngine.objectWillChange.sink { [weak self] _ in self?.objectWillChange.send() }.store(in: &cancellables)
    }
    ...
    private func wireRenderEngine() {
        ...
        renderEngine.frameContextProvider = { [weak self] in
            ...
            return RenderEngine.FrameContext(
                ...
                treble: bands.treble,
                energy: bands.energy,
                faceBoxes: self.cameraEngine.faceBoxes
            )
        }
    ...
    private func value(forInputNode node: GraphNode, context: RenderEngine.FrameContext) -> Float {
        ...
        case "sequencer":
            ...
            return s - floor(s)
        case "faceDetected":
            // AVFoundation's own on-device face detector (no model of ours
            // involved) — 1 the instant a face is in frame, 0 the instant
            // it isn't. A hard on/off rather than a smoothed value; wire it
            // through an Envelope input first if a softer rise/fall reads
            // better for a given patch.
            return cameraEngine.hasFace ? 1 : 0
        default:
            return 0
        }
    }
    ...
    func savePatch(named name: String) {
        ...
        do {
            ...
            print("Patchlume: failed to save patch \"\(name)\": \(error)")
        }
    }
