...
final class GraphStore: ObservableObject {
    ...
    /// Swaps a node's subtype in place, keeping its id/position/connections
    /// — used by `AutoDirectorEngine` to switch a generator between "camera"
    /// and "ndiSource" live (same pipeline-lookup-by-subtype mechanism
    /// `RenderEngine` already re-runs every frame, so no other plumbing is
    /// needed). Not for general use: swapping to an unrelated subtype would
    /// leave stale/irrelevant parameter values in place, which is fine here
    /// because camera and ndiSource share the same p0/p1 (Exposure/
    /// Saturation) meaning and both have p2/p3 overwritten every frame by
    /// RenderEngine's cover-fit logic regardless of value.
    func updateSubtype(nodeID: UUID, subtype: String, recordUndo: Bool = true) {
        guard let index = graph.nodes.firstIndex(where: { $0.id == nodeID }) else { return }
        if recordUndo { snapshotForUndo() }
        graph.nodes[index].subtype = subtype
    }
