...
struct ContentView: View {
    ...
    @State private var showingSceneBank = false
    @State private var showingDirectorSettings = false
    @State private var showingShareSheet = false
    @State private var showingNDIInfo = false
    ...
    var body: some View {
                ...
                VStack(spacing: 0) {
                    if viewModel.store.previewHidden {
                        ...
                        ZStack {
                            ...
                            .overlay(alignment: .topLeading) {
                                if viewModel.autoDirector.isActive {
                                    Label("Auto Director · \(viewModel.autoDirector.currentSceneName)", systemImage: "wand.and.stars")
                    ...
                    ZStack(alignment: .bottomTrailing) {
                        GraphCanvasView()
                            ...
                            .environmentObject(viewModel.ndiSourceEngine)
            ...
            .toolbar {
                ...
                ToolbarItemGroup(placement: .topBarTrailing) {
                    Menu {
                        ...
                        Button {
                            if viewModel.ndiOutputManager.isActive {
                                viewModel.ndiOutputManager.stop()
                            } else {
                                viewModel.ndiOutputManager.start(size: viewModel.renderEngine.recordingResolution)
                                showingNDIInfo = true
                            }
                        } label: {
                            Label(viewModel.ndiOutputManager.isActive ? "Stop NDI Output" : "Start NDI Output",
                                  systemImage: viewModel.ndiOutputManager.isActive ? "antenna.radiowaves.left.and.right.slash" : "antenna.radiowaves.left.and.right")
                        }
                    } label: {
                    ...
                    // Split out from "Tools" above once it grew past the
                    // same mis-tap-prone length the File/Tools split was
                    // already fixing for — everything to do with the Auto
                    // Director (manual Scene Bank included, since it's the
                    // hands-on counterpart to the same feature) gets its own
                    // short menu instead of padding Tools back out again.
                    Menu {
                        Button {
                            showingSceneBank = true
                        } label: {
                            Label("Scene Bank", systemImage: "square.grid.2x2")
                        }
                        Button {
                            if viewModel.autoDirector.isActive {
                                viewModel.autoDirector.stop()
                            } else {
                                viewModel.autoDirector.start()
                            }
                        } label: {
                            Label(viewModel.autoDirector.isActive ? "Stop Auto Director" : "Start Auto Director",
                                  systemImage: viewModel.autoDirector.isActive ? "wand.and.stars.inverse" : "wand.and.stars")
                        }
                        Button {
                            showingDirectorSettings = true
                        } label: {
                            Label("Director Settings", systemImage: "slider.horizontal.below.rectangle")
                        }
                    } label: {
        ...
        .sheet(isPresented: $showingNDIInfo) {
            VStack(spacing: 16) {
                Image(systemName: "antenna.radiowaves.left.and.right")
                    .font(.system(size: 40))
                    .foregroundStyle(.green)
                Text("NDI Output Active").font(.headline)
                Text("Patchlume is now broadcasting as an NDI source named “Patchlume” on your local network — pick it up in OBS, Resolume, vMix, or any other NDI-compatible app on the same Wi-Fi/network.")
                    .font(.subheadline)
                    .multilineTextAlignment(.center)
                    .foregroundStyle(.secondary)
                Link("Powered by NDI® — ndi.video", destination: URL(string: "https://ndi.video")!)
                    .font(.footnote)
                Button("Done") { showingNDIInfo = false }
        ...
        .fullScreenCover(isPresented: $showingSceneBank) {
            SceneBankView(crossfader: viewModel.sceneCrossfader, onClose: { showingSceneBank = false })
        }
        .sheet(isPresented: $showingDirectorSettings) {
            DirectorSettingsView(autoDirector: viewModel.autoDirector)
        }
        ...
        .sheet(isPresented: $showingShareSheet, onDismiss: { viewModel.videoRecorder.lastExportURL = nil }) {
            if let url = viewModel.videoRecorder.lastExportURL {
                VStack(spacing: 16) {
                    ...
                    ShareLink(item: url, preview: SharePreview("Patchlume Recording")) {
