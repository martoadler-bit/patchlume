...
struct NodeContainerView: View {
    ...
    @EnvironmentObject var ndiSourceEngine: NDISourceEngine
    let node: GraphNode
    let scale: CGFloat
    ...
    @State private var showingNDISourcePicker = false
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingNDISourcePicker) {
            NDISourcePickerView(engine: ndiSourceEngine)
        }
    }
    ...
    private var header: some View {
        HStack {
            ...
            Menu {
                ...
                if node.kind == .generator, node.subtype == "ndiSource" {
                    Button {
                        showingNDISourcePicker = true
                    } label: {
                        Label("Choose NDI Source", systemImage: "antenna.radiowaves.left.and.right")
                    }
                }
                Button {
        ...
        if node.kind == .generator, node.subtype == "text" {
            Button {
                ...
                HStack(spacing: 6) {
                    ...
                    Text((node.textContent?.isEmpty == false ? node.textContent! : "PATCHLUME"))
                        .lineLimit(1)
                }
            ...
            Button {
                ...
                HStack(spacing: 6) {
                    Image(systemName: "antenna.radiowaves.left.and.right")
                        .foregroundStyle(ndiSourceEngine.isConnected ? .green : .secondary)
                    Text(ndiSourceEngine.currentSourceName ?? "Choose Source")
                        .lineLimit(1)
                }
                .font(.system(size: 11, weight: .medium))
                .foregroundStyle(.primary)
