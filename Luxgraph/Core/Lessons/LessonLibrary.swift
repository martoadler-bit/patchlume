...
/// `LessonLibrary`; every word of content here is Luxgraph-specific (node
...
enum LessonLibrary {
    ...
    private static let firstGenerator: Lesson = {
        Lesson(
            ...
            icon: "sparkles",
            steps: [
                LessonStep(text: "Every Luxgraph graph starts with a Generator — a node that produces a texture from scratch, with no input needed. Let's build the simplest possible graph: one Generator feeding straight into Output."),
                LessonStep(
        ...
        Lesson(
            ...
            steps: [
                ...
                LessonStep(
                    ...
                    buildActions: [
                        .addNode("cam", .generator, "camera", caption: "Camera — Luxgraph will ask for camera permission the first time a graph like this one actually runs."),
