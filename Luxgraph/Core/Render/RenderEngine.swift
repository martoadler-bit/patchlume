...
final class RenderEngine: NSObject, MTKViewDelegate {
    ...
    override init() {
        guard let device = MTLCreateSystemDefaultDevice(), let queue = device.makeCommandQueue() else {
            fatalError("Luxgraph requires a Metal-capable device.")
        }
        self.device = device
    ...
    private func makeTexture() -> MTLTexture {
        ...
        guard let texture = device.makeTexture(descriptor: descriptor) else {
            fatalError("Luxgraph: failed to allocate an offscreen render texture.")
        }
        return texture
    }
    ...
    /// and hands the composited result to `videoRecorder`. Called by
    /// `VideoRecorder`'s own capture timer while recording, not by the
    /// regular per-view draw loop.
    func captureFrameForRecording() {
        guard let recorder = videoRecorder, recorder.isRecording,
              let graphProvider, let frameContextProvider,
              let commandBuffer = commandQueue.makeCommandBuffer() else { return }
        ...
        recorder.appendFrame(texture: readTexture(for: sourceID), commandBuffer: commandBuffer)
        commandBuffer.commit()
    }
    ...
    private func render(node: GraphNode, graph: Graph, context: FrameContext, commandBuffer: MTLCommandBuffer) {
        ...
        case .modifier:
            ...
            if node.subtype == "feedback" || node.subtype == "trails" {
